...
本方案不仅依赖 Ollama，还支持 **LM Studio** 和 **Lemonade Server**，这些平台都支持 NPU（XDNA 加速器）。NPU 可以在 AI 推理中分担部分工作负载，与 GPU 形成互补。

## 为什么添加 dGPU？

添加 dGPU 的目的不是减少 iGPU 的内存，而是**为 iGPU 分担重负载**。当 iGPU 在处理大型模型时，dGPU 可以同时运行另一个模型或处理其他推理任务，从而**整体加速模型推理能力**。两块 GPU 各司其职，协同工作，实现更高的总吞吐量。
